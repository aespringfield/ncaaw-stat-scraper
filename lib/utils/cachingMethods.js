...
const getStatsFromCache = (name, stats, path, cacheValidHours = 2, skipIfStatsMissing = true) => {
    return getCachedPlayerStats(name, path, cacheValidHours).then((cachedPlayerStats) => {
        ...
        return statMissingFromCache && skipIfStatsMissing ? null : statsObj;
    });
}
