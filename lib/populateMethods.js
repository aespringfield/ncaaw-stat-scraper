...
const { getSheets, getSheetName, listPlayers, listHeadingsAndPlayers, setPlayerData, authorize } = require('./sheetsMethods');
...
//     console.log(addColumnsToHeadings(headings))
// })

function convertIndexToColumn(index, columnString='') {
    const alphabet = 'ABCDEFGHIJKLMNOPQRSTUVWXYZ'.split('');
    const remainder = index % 26 > 0 ? index % 26 : 26;
    const newColumnString = alphabet[remainder - 1] + columnString;
    const dividend = (index - remainder)/26
    return dividend > 0 ? convertIndexToColumn(dividend, newColumnString) : newColumnString;
}

function addColumnsToHeadings(headings) {
    return headings.map((heading, index) => {
        console.log('index', index)
        return { heading, column: convertIndexToColumn(index + 1) }
    })
}

function filterHeadings(headings, statList) {
}
