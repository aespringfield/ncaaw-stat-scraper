...
const { getSheets, getSheetName, listHeadingsAndPlayers, setPlayerData, authorize } = require('./sheetsMethods');
...
function populateFrom(source) {
    ...
    readPlayers()
        ...
        .then(({ players, headings }) => {
            const columns = buildRelevantColumns(headings);
            return scrapeMethod(players, { statList: stats, year: config.STATS_YEAR })
