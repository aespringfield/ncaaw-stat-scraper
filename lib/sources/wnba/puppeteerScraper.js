...
const { cachePlayerStats, getStatsFromCache, getgetCachedPlayerStats, getCachedPlayerStatsPath } = require('../../utils/cachingMethods');
...
const navigateToPlayerPage = async (page, playerName) => {
  ...
  //
  // // The filter to select the regular season is wonky (seemed to respond only to mousedown in Safari, but wouldn't
  // // work in Chrome; could make the select appear by focusing on it and hitting arrow up, but then further arrow up
  // // worked only when applied manually, not via Puppeteer), so just grab the URL for the player page and the query param
  // // onto it that will make the stats be for the regular season
  // const playerPageUrl = page.url()
  // await page.goto(`${playerPageUrl}?SeasonType=Regular%20Season`);

  await new Promise(resolve => setTimeout(resolve, 5000))
}

// const getStatsFromCache = (name, stats, year) => {
//   return getCachedPlayerStats(name, getCachedPlayerStatsPath(name, `${__dirname}/cachedPlayerStats-${year}`)).then((cachedPlayerStats) => {
...
const getStatsForPlayer = async (page, playerName, stats, year) => {
  ...
  const scrapedStats = await scrapeStatsForPlayer(page, tweakName(playerName), stats, year)
