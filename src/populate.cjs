...
const { getSheets, getSheetName, listHeadingsAndPlayers, setPlayerData, listHeadingsAndPlayersByYear, authorize } = require('./sheets.cjs');
const config = require('./config');
const WNBA_STATS = require('./wnbaStats')
const NCAA_STATS = require('./ncaaStats')
const HS_STATS = require('./highSchoolStats')
const SCRAPERS = {
    NCAA: require(`./scrapers/ncaa/scraper.cjs`),
    HHS: require(`./scrapers/hhs/scraper.cjs`),
    WNBA: require(`./scrapers/wnba/scraper.cjs`),
    HG: require('././scrapers/hg/scraper.cjs')
};
...
function populateFrom(source) {
    ...
    readPlayers()
        ...
        .then(({ players, playersByYear, headings }) => {
            ...
            return runScraper(scraper, playersByYear, stats)
...
/**
 * Runs a scraper function for each year in the provided data, sequentially.
 *
 * The scraper function is applied to each year's players, and the results are
 * accumulated into a single array. The scraper function is called with the
 * players for a particular year and the stats to be retrieved.
 *
 * @param {Function} scrapeMethod - An asynchronous function to scrape player data.
 * @param {Object} playersByYear - An object where keys are years and values are arrays
 *   of player names for that year.
 * @param {Array<string>} statList - An array of strings representing the stats to retrieve
 *   for each player.
 *
 * @returns {Promise<Array>} A Promise that resolves to an array containing all players' stats
 *   across all years.
 */
function runScraper(scraper, playersByYear, statList) {
    return Object.keys(playersByYear).reduce((playersPromiseChain, year) => {
        return playersPromiseChain.then((accumulatedPlayers) => {
            return scraper.scrape(playersByYear[year], { statList, year })
                .then((players) => {
...
function readPlayers() {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (config.STATS_YEAR === 'ROOKIE' || config.STATS_YEAR === 'LAST_COLLEGE') {
            yearOffset = 0;
        ...
        if (config.STATS_YEAR === 'LAST_COLLEGE' || config.STATS_YEAR === 'ROOKIE' || config.STATS_YEAR === 'SOPHOMORE' || config.STATS_YEAR === 'TWO YEAR VET' || config.STATS_YEAR === 'THREE YEAR VET') {
